...
export const DEFAULT_PROJECT_DASHBOARD_PROVIDERS: ProjectDashboardProvider[] = [
  ...
  },
  {
    ...
    title: "Mailery",
    command: "mailery",
    args: ["status", "project-panel", "--project", "{project}", "--limit", "20", "--json", "--contract"],
    optional: true,
...
function actionsPanel(project: Workspace, generatedAt: string): ProjectPanel {
  return ProjectPanelSchema.parse({
    ...
    items: [
      {
        ...
        status: "dry-run/default",
        priority: "low",
        resourceRefs: [{ kind: "action", id: "projects.dashboard.snapshot", name: "Refresh snapshot", tags: ["read-only"] }],
      },
      {
    ...
    actions: [
      { kind: "action", id: "projects.dashboard.snapshot", name: "Refresh snapshot", tags: ["read-only"] },
      { kind: "action", id: "projects.dashboard.validate", name: "Validate dashboard", tags: ["read-only"] },
      { kind: "action", id: "projects.dashboard.serve", name: "Serve dashboard", tags: ["read-only"] },
    ],
  });
}
