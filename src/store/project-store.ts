...
import {
  ...
  upsertProjectCanvas as dbUpsertProjectCanvas,
...
export interface ProjectStore {
  ...
  upsertCanvas(project: Workspace, input: UpsertProjectCanvasInput, ctx?: MutationContext): Promise<ProjectCanvas>;
...
function rethrowProjectHttpError(error: unknown): never {
  if (error instanceof HasnaHttpError && error.body && typeof error.body === "object") {
    const body = error.body as Record<string, unknown>;
    const rawError = body["error"];
    const errorRecord = rawError && typeof rawError === "object" ? rawError as Record<string, unknown> : null;
    const code = errorRecord?.["code"];
    if (typeof code === "string" && PROJECT_CONTEXT_ERROR_CODES.includes(code as ProjectContextErrorCode)) {
      const rawProject = body["project"];
      const project = rawProject && typeof rawProject === "object"
        ? rawProject as { id: string; slug: string; status: string }
        : undefined;
      throw new ProjectContextError(
        code as ProjectContextErrorCode,
        typeof errorRecord?.["message"] === "string" ? errorRecord["message"] : "Projects authority rejected the request",
        { status: error.status, project, cause: error },
      );
    }
  }
  throw error;
...
function assertProjectMutationAllowed(
  project: Workspace | null,
  idOrSlug: string,
  transition?: Workspace["status"],
): asserts project is Workspace {
  if (!project) {
    throw new ProjectContextError("PROJECT_NOT_FOUND", `Project not found: ${idOrSlug}`, { status: 404 });
  }
  ...
  if (project.status === "archived" && transition !== "active" && transition !== "deleted") {
...
class LocalProjectStore implements ProjectStore {
  ...
  async resolveTargetResolution(
    target: string | undefined,
    options: ProjectResolverOptions = {},
  ): Promise<CanonicalProjectTargetResolution> {
    const database = options.db ?? getDatabase();
    return resolveCanonicalProjectTarget(target, {
      ...options,
      db: database,
      authority: {
        mode: "local",
        owner: APP,
        storage: "sqlite",
        getProject: async (idOrSlug) => dbResolveWorkspace(idOrSlug, database),
        listProjects: async (query) => dbListWorkspaces({ query, limit: 200 }, database),
      },
    });
  }

  async resolveTarget(target: string | undefined, options?: ProjectResolverOptions): Promise<Workspace> {
    return (await this.resolveTargetResolution(target, options)).project;
  }

  async createProject(input: CreateWorkspaceInput): Promise<Workspace> {
    if (input.primary_path) {
      try {
        const resolution = await this.resolveTargetResolution(input.primary_path, { intent: "read" });
        throw new ProjectContextError("PROJECT_ALREADY_REGISTERED", "Project path is already registered", {
          status: 409,
  ...
  async updateProject(id: string, patch: UpdateWorkspaceInput): Promise<Workspace> {
    assertProjectMutationAllowed(dbResolveWorkspace(id), id, patch.status);
  ...
  async archiveProject(id: string, ctx?: MutationContext): Promise<Workspace> {
    assertProjectMutationAllowed(dbResolveWorkspace(id), id, "archived");
  ...
  async unarchiveProject(id: string, ctx?: MutationContext): Promise<Workspace> {
    assertProjectMutationAllowed(dbResolveWorkspace(id), id, "active");
  ...
  async deleteProject(id: string, opts: { hard?: boolean }, ctx?: MutationContext): Promise<DeleteProjectResult> {
    assertProjectMutationAllowed(dbResolveWorkspace(id), id, "deleted");
  ...
  async upsertCanvas(project: Workspace, input: UpsertProjectCanvasInput, ctx?: MutationContext): Promise<ProjectCanvas> {
    return withLock(project.id, ctx, "project canvas upsert", () => dbUpsertProjectCanvas(project, input));
  }
...
class ApiProjectStore implements ProjectStore {
  ...
  private readonly authorityStorage: "cloud" | "self-hosted";

  constructor(client: StorageClient, env: Env) {
    this.client = client;
    this.env = env;
    this.baseUrl = client.baseUrl;
    const requestedMode = [
      env["HASNA_PROJECTS_STORAGE_MODE"],
      env["HASNA_PROJECTS_MODE"],
      env["PROJECTS_STORAGE_MODE"],
      env["PROJECTS_MODE"],
    ].find((value) => value?.trim())?.trim().toLowerCase().replace(/-/g, "_");
    this.authorityStorage = requestedMode === "self_hosted" ? "self-hosted" : "cloud";
  }
  ...
  private openLocatorDatabase(options: ProjectResolverOptions): { db?: Database; close: boolean } {
    if (options.db) return { db: options.db, close: false };
    const home = this.env["HOME"] || this.env["USERPROFILE"] || homedir();
    const path = this.env["HASNA_PROJECTS_DB_PATH"]
      || this.env["HASNA_WORKSPACES_DB_PATH"]
      || join(home, ".hasna", "projects", "projects.db");
    if (!existsSync(path)) return { close: false };
    try {
      const db = new Database(path, { readonly: true });
      const identityTable = db.query(
        "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'workspace_identity_bindings'",
      ).get();
      if (!identityTable) {
        db.close();
        return { close: false };
  ...
  async resolveTargetResolution(
    ...
    const locator = this.openLocatorDatabase(options);
    try {
      return await resolveCanonicalProjectTarget(target, {
        ...options,
        db: locator.db,
        authority: {
          mode: "api",
          owner: APP,
          storage: this.authorityStorage,
          getProject: (idOrSlug) => this.getProject(idOrSlug),
          listProjects: (query) => this.listProjects({ query, limit: 200 }),
        },
      });
    } finally {
      if (locator.close) locator.db?.close();
    }
  }
  ...
  async createProject(input: CreateWorkspaceInput): Promise<Workspace> {
    let identity: Record<string, string | null> | undefined;
    if (input.primary_path) {
      try {
        const resolution = await this.resolveTargetResolution(input.primary_path, { intent: "read" });
        throw new ProjectContextError(
          ...
          { status: 409, project: resolution.project },
        );
      } catch (error) {
        if (isProjectContextError(error) && error.code === "PROJECT_NOT_FOUND") {
          // No marker or machine binding exists, so an explicit create may proceed.
        } else {
          throw error;
        }
      }
      const owner = this.env["HASNA_MACHINE_ID"]
        || this.env["HASNA_STATION_ID"]
        || this.env["MACHINE_ID"]
        || this.env["STATION_ID"]
        || this.env["HOSTNAME"]
        || projectLocationOwnerId();
      identity = {
        ...
        real_path: canonicalProjectLocationPath(input.primary_path),
        logical_path: input.primary_path,
        station_id: this.env["HASNA_STATION_ID"] || this.env["STATION_ID"] || null,
        machine_id: this.env["HASNA_MACHINE_ID"] || this.env["MACHINE_ID"] || owner,
      };
    }
    try {
  ...
  async upsertCanvas(): Promise<ProjectCanvas> {
    throw new LocalOnlyOperationError("upsert project canvas");
  }
...
export interface ProjectStoreTargetContext {
  db?: Database;
  store?: ProjectStore;
}

/**
 * Select the authority for a target-taking surface.
 *
 * An explicitly injected Store always wins. Otherwise an explicitly injected
 * SQLite database is an explicit local-authority contract, not merely a
 * locator for whatever process-wide remote authority happens to be selected.
 * Normal CLI/MCP calls inject neither and therefore retain the configured
 * hybrid local-locator -> remote-authority behavior.
 */
export function resolveProjectStoreForTarget(context: ProjectStoreTargetContext = {}): ProjectStore {
  if (context.store) return context.store;
  if (context.db) return new LocalProjectStore();
...
export function resolveProjectStore(
  ...
  const store: ProjectStore =
    resolved.transport === "cloud-http" ? new ApiProjectStore(resolved.client, env) : new LocalProjectStore();
